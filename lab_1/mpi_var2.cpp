...
double* Matrix_by_vector(double *M, double *V, int rank, int size, int* block_si                                                                                                                zes, int* offset_posit)
{
...
//print vector
void print_vector(double* vector, int rank, int size, int* part_sizes){
        for(int proc = 0; proc < size; proc++){
                MPI_Barrier(MPI_COMM_WORLD);
                if(rank == proc){
                        for(int i = 0; i < part_sizes[rank]; i++){
                                printf("%0.4f\n", vector[i]);
                        }
                }
        }
...
double get_chisl_tau(double* Ay, double* Y, int rank, int* block_sizes){
        ...
        MPI_Allreduce(&part_chisl, &chisl, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORL                                                                                                                D);
        return chisl;
}
...
double* Minimal_Nevazki(double *A, double *b, int rank, int size, int* block_siz                                                                                                                es, int* offset_posit)
{
    ...
    while(1){
        double* Ax = Matrix_by_vector(A, X, rank, size, block_sizes, offset_posi                                                                                                                t);

        double* Y =  differ_vectors(Ax, b, rank, block_sizes);

        double* Ay = Matrix_by_vector(A, Y, rank, size, block_sizes, offset_posi                                                                                                                t);
...
int main(int argc, char **argv) {
    ...
    //print_vector(b, rank, size, block_sizes);
    ...
    print_vector(X, rank, size, block_sizes);
