...
int main(int argc, char** argv) {
    ...
    while(true) {
        ...
        double tmp_delta = update_layer(base_z, 1, values, tmp_values, hx, hy, hz);
        max_delta = max(max_delta, tmp_delta);

        tmp_delta = update_layer(base_z, part_z, values, tmp_values, hx, hy, hz);
        max_delta = max(max_delta, tmp_delta);

        MPI_Request rq[4];

        if (rank != 0) {
            MPI_Isend(tmp_values+Nx*Ny, Nx*Ny, MPI_DOUBLE, rank-1, 123, MPI_COMM_WORLD, &rq[0]);
        ...
        memcpy(values, tmp_values, (part_z+2)*Nx*Ny*sizeof(double));
    ...
    MPI_Gather(values+Nx*Ny, part_z*Nx*Ny, MPI_DOUBLE, results, part_z*Nx*Ny, MPI_DOUBLE, 0, MPI_COMM_WORLD);
